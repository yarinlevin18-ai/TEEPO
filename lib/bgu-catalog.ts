/**
 * BGU Catalog — client-side loader.
 *
 * The backend was meant to serve catalog data (tracks, departments, courses)
 * from Supabase, but those tables are empty. Since this is static reference
 * data (BGU shnaton 2024-25), we bundle it with the frontend and load it
 * lazily from /public/bgu-catalog.json.
 *
...
async function loadCatalog(): Promise<CatalogFile> {
  if (!_cache) {
    _cache = (async () => {
      const res = await fetch('/bgu-catalog.json', { cache: 'force-cache' })
 ...
 * bgu_tracks table so we compute it here instead, combining local track data
